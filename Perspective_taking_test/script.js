...
function saveResultsToFile() {
    const resultsString = JSON.stringify(results, null, 2); // Convert results array to a formatted JSON string
    const filename = "results.txt"; // Set the filename
    const a = document.createElement('a'); // Create a new <a> element
    a.href = "data:text/plain;charset=utf-8," + encodeURIComponent(resultsString); // Set the href attribute of the <a> element to a data URI containing the results
